...
export const Personalize = <P extends Object>({
  ...
  const { loading, variant, isPersonalized, audience } = usePersonalize<Baseline<P>>(
  ...
  return <TrackHasSeenComponent variant={variant} audience={audience} isPersonalized={isPersonalized}>
    <Component {...variant} key={`${audience.id}-${variant.id}`} ninetailed={{ isPersonalized, audience }} />
